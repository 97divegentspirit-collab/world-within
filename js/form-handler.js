...
function selectEmotion(value, el) {
  ...
  // Retints the background particles/glow to match the chosen atmosphere,
  // and remembers it so the homepage can restore the same feel later.
  document.body.setAttribute('data-emotion', value);
  try { localStorage.setItem('worldWithin_emotion', value); } catch (e) {}
}
...
async function submitProfile() {
  ...
  try {
    ...
    // Fill in the persistent confirmation near the Thank You section too,
    // so it's visible even if someone scrolls past the status line above.
    var finalConfirm = document.getElementById('journey-final-confirmation');
    if (finalConfirm) {
      finalConfirm.textContent = '✓ Your information has been received. Thank you for trusting me with it.';
      finalConfirm.classList.add('visible');
    }

    // Mark the button as done and keep it disabled — a successful
    // submission should not be repeatable by accident.
    if (btn) {
